...
class CreateUserRolesTable extends Migration
{
    public function up(): void
    {
        ...
        $fk1Sql = $this->addForeignKey('user_roles', 'user_id', 'users', 'id', 'CASCADE');
        if ($fk1Sql !== null) {
            $this->execute($fk1Sql);
        }
        
        $fk2Sql = $this->addForeignKey('user_roles', 'role_id', 'roles', 'id', 'CASCADE');
        if ($fk2Sql !== null) {
            $this->execute($fk2Sql);
        }
        
        // Add unique constraint to prevent duplicate user-role pairs
        $this->execute('CREATE UNIQUE INDEX idx_user_role_unique ON user_roles (user_id, role_id)');
    }
