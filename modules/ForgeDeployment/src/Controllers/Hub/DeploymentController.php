  ...
  public function saveConfig(Request $request): Response
  {
    ...
    if ($updateType === 'post_deployment_commands') {
      ...
      try {
        ...
        if (!$result) {
          $configPath = $this->deploymentHubService->getConfigPath() ?? BASE_PATH . '/forge-deployment.php';
          return $this->jsonResponse([
            'success' => false,
            'message' => 'Failed to save commands - check file permissions. Path: ' . $configPath,
          ], 500);
        }
      } catch (\Exception $e) {
        return $this->jsonResponse([
          'success' => false,
          'message' => 'Error saving commands: ' . $e->getMessage(),
        ], 500);
      }
    ...
    if ($updateType === 'env_vars') {
      ...
      try {
        ...
        if (!$result) {
          ...
          return $this->jsonResponse([
            'success' => false,
            'message' => 'Failed to save environment variables - check file permissions. Path: ' . $configPath,
          ], 500);
        }
      } catch (\Exception $e) {
        return $this->jsonResponse([
          'success' => false,
          'message' => 'Error saving environment variables: ' . $e->getMessage(),
        ], 500);
      }
    ...
    $existingConfig = $this->deploymentHubService->getRawDeploymentConfig() ?? [];

    $mergedConfig = [
      'server' => array_merge($existingConfig['server'] ?? [], $config['server'] ?? []),
      'provision' => array_merge($existingConfig['provision'] ?? [], $config['provision'] ?? []),
      'deployment' => array_merge($existingConfig['deployment'] ?? [], $config['deployment'] ?? []),
    ];
    ...
    $result = $this->deploymentHubService->saveDeploymentConfig($mergedConfig);
    if (!$result) {
      return $this->jsonResponse([
        'success' => false,
